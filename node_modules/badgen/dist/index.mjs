...
function n(e) {
	...
	let t = parseInt(e.slice(0, 2), 16), n = parseInt(e.slice(2, 4), 16), r = parseInt(e.slice(4, 6), 16);
	return (t * 299 + n * 587 + r * 114) / 255e3;
}
function r(e) {
	return n(e) <= .69 ? {
		textColor: "#fff",
...
function i({ label: n, subject: i, status: u, color: d = "blue", style: f, icon: p, iconWidth: m = 13, labelColor: h = "555", scale: g = 1 }) {
	if (l(typeof u == "string", "<status> must be string"), n = n === void 0 ? i : n, !n && !p) return a({
		status: u,
		color: d,
		style: f,
		scale: g
	});
	d = t[d] || d, h = t[h] || h, m *= 10;
	let { textColor: _, shadowColor: v } = r(h), { textColor: y, shadowColor: b } = r(d), x = p ? n?.length ? m + 30 : m - 18 : 0, S = p ? x + 50 : 50, C = n ? e(n) : 0, w = e(u), T = C + 100 + x, E = w + 100, D = T + E, O = p ? " xmlns:xlink=\"http://www.w3.org/1999/xlink\"" : "", k = s(5), A = s(5);
	n = n ? o(n) : "", u = o(u), d = o(d), h = o(h), p &&= o(p);
	let j = c({
		label: n,
		status: u
	});
	return f === "flat" ? `<svg width="${g * D / 10}" height="${g * 20}" viewBox="0 0 ${D} 200" xmlns="http://www.w3.org/2000/svg"${O} role="img" aria-label="${j}">
  <title>${j}</title>
  <g>
...
function a({ status: n, color: i = "blue", style: a, scale: c = 1 }) {
	l(typeof n == "string", "<status> must be string"), i = t[i] || i || t.blue;
	let { textColor: u, shadowColor: d } = r(i), f = e(n), p = f + 115, m = s(5), h = s(5);
	return n = o(n), i = o(i), a === "flat" ? `<svg width="${c * p / 10}" height="${c * 20}" viewBox="0 0 ${p} 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${n}">
  <title>${n}</title>
  <g>
...
function s(e) {
	let t = "";
	for (let n = 0; n < e; n++) t += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz".charAt(Math.floor(Math.random() * 52));
	return t;
}
...
//#endregion
export { i as badgen, i as default, e as calcWidth };
